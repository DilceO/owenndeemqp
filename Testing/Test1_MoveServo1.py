...
class DX_XM430_W350:
    ...
    def stopConnection(self):
        packetHandler.closePort(self.port_num)

    def getJointReadings(self):
        readings = [0, 0, 0]

        cur_pos = float(self.readWriteByte(self.POS_LEN, self.CURR_POSITION))
        cur_vel = float(self.readWriteByte(self.VEL_LEN, self.CURR_VELOCITY))
        cur_curr = float(self.readWriteByte(self.CURR_LEN, self.CURR_CURRENT))
    ...
    def checkPacket(self, addr, msg=None):
        dxl_comm_result = packetHandler.getLastTxRxResult(self.port_num, self.PROTOCOL_VERSION)
        dxl_error = packetHandler.getLastRxPacketError(self.port_num, self.PROTOCOL_VERSION)
        ...
        if dxl_comm_result != self.COMM_SUCCESS:
            print(f'[addr:{addr}] {packetHandler.getTxRxResult(self.PROTOCOL_VERSION, dxl_comm_result)}\n')
            raise Exception("Communication Error: See above.")
        elif dxl_error != 0:
            print(f'[addr:{addr}] {packetHandler.getRxPacketError(self.PROTOCOL_VERSION, dxl_error)}\n')
    ...
    def readWriteByte(self, n, addr, msg=None):
        if msg is not None:
            ...
            if n == 1:
                ...
                packetHandler.write1ByteTxRx(self.port_num, self.PROTOCOL_VERSION, self.id, addr, msg)
            elif n == 2:
                msg = ctypes.c_uint16(msg).value
                packetHandler.write2ByteTxRx(self.port_num, self.PROTOCOL_VERSION, self.id, addr, msg)
            elif n == 4:
                msg = ctypes.c_uint32(msg).value
                packetHandler.write4ByteTxRx(self.port_num, self.PROTOCOL_VERSION, self.id, addr, msg)
            else:
            ...
            if n == 1:
                result = packetHandler.read1ByteTxRx(self.port_num, self.PROTOCOL_VERSION, self.id, addr)
                result = ctypes.c_int8(result).value
            elif n == 2:
                result = packetHandler.read2ByteTxRx(self.port_num, self.PROTOCOL_VERSION, self.id, addr)
                result = ctypes.c_int16(result).value
            elif n == 4:
                result = packetHandler.read4ByteTxRx(self.port_num, self.PROTOCOL_VERSION, self.id, addr)
