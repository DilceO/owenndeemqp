...
class Robot(OM_X_arm):
    ...
    def __init__(self):
        ...
        self.mDHTable = [[0, self.mDim[0], 0, -90],
                         [(asind(self.mOtherDim[1]/self.mOtherDim[0])) - 90, 0, self.mDim[1], 0],
                         [(90 - asind(self.mOtherDim[1]/self.mOtherDim[0])), 0, self.mDim[2], 0],
                         [0, 0, self.mDim[3], 0]]
    ...
    def writeJoints(self, goals):
        goals = np.mod(np.round(goals * DX_XM430_W350.TICKS_PER_DEG + DX_XM430_W350.TICK_POS_OFFSET), DX_XM430_W350.TICKS_PER_ROT)
    ...
    def writeCurrents(self, currents):
        currentInTicks = np.round(currents * DX_XM430_W350.TICKS_PER_mA)
    ...
    def getJointsReadings(self):
        ...
        readings[0, :] = (self.bulkReadWrite(DX_XM430_W350.POS_LEN, DX_XM430_W350.CURR_POSITION) - DX_XM430_W350.TICK_POS_OFFSET) / DX_XM430_W350.TICKS_PER_DEG
        readings[1, :] = self.bulkReadWrite(DX_XM430_W350.VEL_LEN, DX_XM430_W350.CURR_VELOCITY) / DX_XM430_W350.TICKS_PER_ANGVEL
        readings[2, :] = self.bulkReadWrite(DX_XM430_W350.CURR_LEN, DX_XM430_W350.CURR_CURRENT) / DX_XM430_W350.TICKS_PER_mA
        return readings
    ...
    def writeVelocities(self, vels):
        vels = np.round(vels * DX_XM430_W350.TICKS_PER_ANGVEL)
